var stylepointsGame = function() {
    ...
    var fourImagesCopy = {
        ...
        "resultText1": "Blake Lively recently twisted her long locks into the sweetest heart half-up ‘do. And now you can too… ",
        "resultTextHeadline1": "You’re Creative And Enjoy Experimenting With New Hairstyles",
        "url1": "http://www.look.co.uk/beauty/blake-lively-heart-shaped-hair-575753",
        "resultText2": "You’re sassy and fun, which can be reflected in your hairstyle. Don’t be afraid of the braid – our very own beauty writer shows you how to perfect the fishtail… ",
        "resultTextHeadline2": "Make A Style Statement With Plaits",
        "url2":"http://www.look.co.uk/beauty/hun-aka-third-day-hair-fix-551858 ",
        "resultText3": "Third day hair? No problem, this super speedy style will transform your locks in no time.",
        "resultTextHeadline3": "You Have A Unique Style, Make Like A Trend-Setter And Do The Hun",
        "url3":"http://www.look.co.uk/beauty/watch-how-to-fishtail-plait-tutorial-554954",
        "resultText4": "Whether for work or play, if you don’t have much time, these simple styles will have you looking and feeling your best… ",
        "resultTextHeadline4": "Easy Peasy Hairstyles You Can Do At Home ",
        "url4": " http://www.look.co.uk/beauty/easy-peasy-party-hairstyles-you-can-do-200835",
    }
    ...
    var presentEmailScreen = function() {
        ...
        switch (parseInt(answerGroupThree[0],10)){
            case 0:
        ...
        var emailSubmitForm = $.create('a', {
            id: 'emailFormButton',
            href: fourImagesCopy['url' + (parseInt(answerGroupThree[0]) + 1)],
    ...
    var buildGameDom = function() {
        ...
        var imageLabels = ["SHORT TO MEDIUM", "MEDIUM TO LONG", "NATURAL CURLS", "CAN'T BE TAMED!"];
        for (var j = 0; j < 4; j++) {
            var gameImageGridImageContainer = $.create('div', {
                className: 'gameImageGridImageContainer',
                contents: [{
            ...
            gameImageGridImageContainer.addEventListener('click', function(event) {
                ...
                if (answerGroupTwo.length === 0) {
                    // Push id
                    answerGroupTwo.push(selectedImage);
                    for (var i = 0; i < 4; i++) {
        ...
        for (var k = 0; k < 4; k++) {
            ...
            gameImageGridImageContainer.addEventListener('click', function(event) {
                ...
                var selectedImage = event.target.id.substring(event.target.id.length - 1, event.target.id.length);
                console.log(selectedImage, answerGroupThree);
                // If there are no answers in array.
                if (answerGroupThree.length === 0) {
                    ...
                    answerGroupThree.splice(0, 1);
                    for (var i = 0; i < 4; i++) {
                        // Remove any selected icons
                        if ($("#gridThreeSelected" + i).className === "gameImageGridImageSelectedIcon show") {
                            $.set($("#gridThreeSelected" + i), {
                                className: 'gameImageGridImageSelectedIcon'
                            });
                        }

                        if ($("#gridThreeOverlay" + i).className === "gameImageGridImageOverlay") {
                            $.set($("#gridThreeOverlay" + i), {
                                className: 'gameImageGridImageOverlay hidden'
                            });
                        }
                    ...
                    // Check to see if we selected an overlay
                    // if so, switch the selected icon
                    if (event.target.id.indexOf("gridThreeOverlay") === 0) {
                        for (var j = 0; j < 4; j++) {
                            if (j !== Number(selectedImage)) {
                                $.set($('#gridThreeOverlay' + j), {
                                    className: 'gameImageGridImageOverlay'
                                });
                            } else {
                                $.set($('#gridThreeOverlay' + j), {
        ...
        var gameTitle = $.create('div', {
            id: 'gameTitle',
            contents: fourImagesCopy.gameTitle,
            className: 'gameTitle'
        });
